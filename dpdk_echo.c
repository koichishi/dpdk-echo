...
static int run_server()
{
	...
	struct rte_mbuf *buf, *recv_pkt;
	char *buf_ptr;
	const char *pkt_ptr;
	struct rte_ether_hdr *eth_hdr, *pkt_eth_hdr;
	struct rte_ipv4_hdr *ipv4_hdr, *pkt_ipv4_hdr;
	struct rte_udp_hdr *rte_udp_hdr, *pkt_rte_udp_hdr;
	...
	for (;;) {
		...
		buf_ptr = rte_pktmbuf_append(buf, RTE_ETHER_HDR_LEN);
		eth_hdr = (struct rte_ether_hdr *) buf_ptr;
		pkt_ptr = rte_pktmbuf_read(recv_pkt, 0, RTE_ETHER_HDR_LEN, NULL);
		pkt_eth_hdr = (struct rte_ether_hdr *) pkt_ptr;

		rte_ether_addr_copy(&pkt_eth_hdr->dst_addr, &eth_hdr->src_addr);
		rte_ether_addr_copy(&pkt_eth_hdr->src_addr, &eth_hdr->dst_addr);
		eth_hdr->ether_type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4);

		/* IPv4 header */
		buf_ptr = rte_pktmbuf_append(buf, sizeof(struct rte_ipv4_hdr));
		ipv4_hdr = (struct rte_ipv4_hdr *) buf_ptr;
		pkt_ptr = rte_pktmbuf_read(recv_pkt, RTE_ETHER_HDR_LEN, sizeof(struct rte_ipv4_hdr), NULL);
		pkt_ipv4_hdr = (struct rte_ipv4_hdr *) pkt_ptr;
		...
		ipv4_hdr->next_proto_id = IPPROTO_UDP;
		ipv4_hdr->hdr_checksum = 0;
		ipv4_hdr->src_addr = pkt_ipv4_hdr->dst_addr;
		ipv4_hdr->dst_addr = pkt_ipv4_hdr->src_addr;

		/* UDP header + fake data */
		buf_ptr = rte_pktmbuf_append(buf,
				sizeof(struct rte_udp_hdr) + payload_len);
		rte_udp_hdr = (struct rte_udp_hdr *) buf_ptr;
		pkt_ptr = rte_pktmbuf_read(recv_pkt, 
				RTE_ETHER_HDR_LEN + sizeof(struct rte_ipv4_hdr), 
				sizeof(struct rte_udp_hdr), NULL);
		pkt_rte_udp_hdr = (struct rte_udp_hdr *) pkt_ptr;

		rte_udp_hdr->src_port = pkt_rte_udp_hdr->dst_port;
		rte_udp_hdr->dst_port = pkt_rte_udp_hdr->src_port;
		rte_udp_hdr->dgram_len = rte_cpu_to_be_16(sizeof(struct rte_udp_hdr)
				+ payload_len);
		rte_udp_hdr->dgram_cksum = 0;
		memset(buf_ptr + sizeof(struct rte_udp_hdr), 0xAB, payload_len);
